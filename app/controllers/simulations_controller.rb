...
class SimulationsController < ApplicationController
  def search
    ...
    # 가상자산 계산
    unless @asset_history.count == simulation.virtual_assets.count
      simulation.virtual_assets.where("last_modified IS NULL OR last_modified < '#{Time.now - 1.days}'").delete_all
