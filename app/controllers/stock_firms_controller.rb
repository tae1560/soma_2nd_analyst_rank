...
class StockFirmsController < ApplicationController
  ...
  def show
    ...
    if @recent_period and @recent_period.days > 0
      @recommendations = @stock_firm.recommendations.where("in_date > '#{Time.now - @recent_period.days.days}'").order_by("in_date DESC").paginate(:page => params[:page], :per_page => 30)
    else
      @recommendations = @stock_firm.recommendations.order_by("in_date DESC").paginate(:page => params[:page], :per_page => 30)
    end
